...
async function waitForVoiceTail(session, quietMs = 180, maxWaitMs = 700) {
  const start = Date.now();
  while (Date.now() - start < maxWaitMs) {
    const lastChunkAt = session.lastChunkAt || session.startedAt || start;
    if (Date.now() - lastChunkAt >= quietMs) return;
    await sleep(35);
  }
}
let latestSpeech = {
...
async function handleVoiceSession(ws) {
  ...
  await waitForVoiceTail(session);
  ...
  if (pcm.length < 10000) {
    ...
    await sendChatText("Tahan mic sedikit lebih lama.");
    return;
  }
...
function controlPageHtml() {
  return `<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>GemBot Control</title><style>${appShellStyles()}
    ...
    .volRow{display:flex;align-items:center;gap:16px;color:#fff;font-weight:800;font-size:14px;margin-bottom:40px}.volSlider{flex:1;height:8px;border-radius:4px;background:rgba(255,255,255,0.24);appearance:none;outline:0;cursor:pointer;touch-action:none}.volSlider::-webkit-slider-thumb{appearance:none;width:18px;height:18px;border-radius:50%;background:#2e9bff;cursor:pointer;box-shadow:0 0 0 4px rgba(46,155,255,.18)}
    ...
    .playlist{display:grid;gap:8px;max-height:220px;overflow-y:auto;padding-right:10px}.playlist::-webkit-scrollbar{width:10px}.playlist::-webkit-scrollbar-track{background:rgba(255,255,255,0.28);border-radius:12px}.playlist::-webkit-scrollbar-thumb{background:#fff;border-radius:12px}.song{display:grid;grid-template-columns:36px 1fr auto;align-items:center;min-height:48px;border-radius:24px;padding:0 18px;font-size:14px;font-weight:900;color:#fff;transition:background .18s;cursor:pointer;user-select:none;touch-action:pan-y}.song:hover{background:rgba(255,255,255,0.14)}.song.active{background:rgba(255,255,255,0.42);color:#fff}.song small{color:rgba(255,255,255,0.82);font-weight:700;text-align:right}
    ...
    <section class="tabPanel"><div id="pane-expr" class="pane active"><h2 class="title">Kontrol Ekspresi</h2><p class="subtitle">Pilih ekspresi GemBot S3, tampilannya langsung ikut di panel dan layar.</p><div class="gridExpr"><button data-cmd="M0" class="exprBtn"><span>●‿●</span><small>Normal</small></button><button data-cmd="M1" class="exprBtn"><span>⌒▽⌒</span><small>Senang</small></button><button data-cmd="M2" class="exprBtn"><span>♥‿♥</span><small>Love</small></button><button data-cmd="M3" class="exprBtn"><span>＞︿＜</span><small>Marah</small></button><button data-cmd="M4" class="exprBtn"><span>O_O</span><small>Kaget</small></button><button data-cmd="M5" class="exprBtn"><span>-_-</span><small>Ngantuk</small></button><button data-cmd="M6" class="exprBtn"><span>T_T</span><small>Sedih</small></button><button data-cmd="M7" class="exprBtn"><span>✦▽✦</span><small>Excited</small></button><button data-cmd="M8" class="exprBtn"><span>¬‿¬</span><small>Smug</small></button><button data-cmd="M9" class="exprBtn"><span>o_o</span><small>Takut</small></button><button data-cmd="M10" class="exprBtn"><span>u‿u</span><small>Cozy</small></button><button data-cmd="M11" class="exprBtn"><span>@_@</span><small>Pusing</small></button><button data-cmd="M12" class="exprBtn"><span>>:3</span><small>Nakal</small></button><button data-cmd="M13" class="exprBtn"><span>///</span><small>Malu</small></button><button data-cmd="M14" class="exprBtn"><span>◉_◉</span><small>Fokus</small></button><button data-cmd="M15" class="exprBtn"><span>¬_¬</span><small>Bosan</small></button><button data-cmd="M16" class="exprBtn"><span>&gt;_&lt;</span><small>Nope</small></button><button data-cmd="M17" class="exprBtn"><span>♪▽♪</span><small>Party</small></button><button data-cmd="M18" class="exprBtn"><span>˘‿˘</span><small>Lega</small></button><button data-cmd="M19" class="exprBtn"><span>◑_◐</span><small>Curiga</small></button><button data-cmd="M20" class="exprBtn"><span>≧▽≦</span><small>Ketawa</small></button><button data-cmd="M21" class="exprBtn"><span>•_´•</span><small>Tekad</small></button><button data-cmd="M22" class="exprBtn"><span>O▽O</span><small>Wow</small></button><button data-cmd="M23" class="exprBtn"><span>u﹏u</span><small>Meleleh</small></button><button data-cmd="M24" class="exprBtn"><span>✧‿✧</span><small>Delight</small></button><button data-cmd="M25" class="exprBtn"><span>._.</span><small>Guilty</small></button><button data-cmd="M26" class="exprBtn"><span>◌‿◌</span><small>Daydream</small></button><button data-cmd="M27" class="exprBtn"><span>ಠ_ಠ</span><small>Grumpy</small></button><button data-cmd="M28" class="exprBtn"><span>✪▽✪</span><small>Amazed</small></button><button data-cmd="M29" class="exprBtn"><span>ಥ_ಥ</span><small>Nangis</small></button><button data-cmd="M30" class="exprBtn"><span>@﹏@</span><small>Pusing+</small></button><button data-cmd="M31" class="exprBtn"><span>>:D</span><small>Nakal+</small></button><button data-cmd="M32" class="exprBtn"><span>◑_◐</span><small>Curiga+</small></button><button data-cmd="M33" class="exprBtn"><span>⌐■‿■</span><small>Keren</small></button><button data-cmd="M34" class="exprBtn"><span>…_…</span><small>Mikir</small></button><button data-cmd="M35" class="exprBtn"><span>✧▽✧</span><small>Berbinar</small></button><button data-cmd="M36" class="exprBtn"><span>●︿●</span><small>Kecewa</small></button><button data-cmd="M37" class="exprBtn"><span>◣皿◢</span><small>Ngamuk</small></button><button data-cmd="M38" class="exprBtn"><span>¬_¬</span><small>Bosan+</small></button><button data-cmd="M39" class="exprBtn"><span>///‿///</span><small>Malu+</small></button><button data-cmd="M40" class="exprBtn"><span>?﹏?</span><small>Bingung</small></button><button data-cmd="M50" class="exprBtn"><span>♥‿♥</span><small>Touch Love</small></button><button data-cmd="M51" class="exprBtn"><span>＞▽＜</span><small>Laugh</small></button></div></div>
      <div id="pane-sys" class="pane"><h2 class="title">Sistem</h2><p class="subtitle">Kontrol sentuh dan layar GemBot dari web.</p><div class="gridExpr"><button type="button" data-cmd="P" class="exprBtn"><span class="big">TAP</span><small>Buka menu / pilihan berikutnya</small></button><button type="button" data-cmd="O" class="exprBtn"><span class="big">HOLD</span><small>Enter / konfirmasi</small></button><button type="button" data-cmd="C" class="exprBtn"><span class="big">BACK</span><small>Kembali ke wajah</small></button><button type="button" data-cmd="F" class="exprBtn"><span class="big">FACE</span><small>Tampilkan wajah</small></button></div></div>
      <div id="pane-music" class="pane"><h2 class="title">Musik DFPlayer</h2><p class="subtitle">Pilih lagu, atur volume, lalu kontrol playback GemBot.</p><div class="musicNow"><span>Sedang diputar</span><strong id="songTitle">Love Story</strong><small id="songArtist">Taylor Swift - 3:54</small></div><div class="musicControls"><button id="btnPrevSong" class="circle" title="Sebelumnya"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M11 5L4 12l7 7V5z"/><path d="M19 5l-7 7 7 7V5z"/></svg></button><button id="btnPlaySong" class="circle main" title="Play/Pause"><svg id="playIcon" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></button><button id="btnNextSong" class="circle" title="Berikutnya"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M13 5l7 7-7 7V5z"/><path d="M5 5l7 7-7 7V5z"/></svg></button><button id="btnStopAudio" class="circle" title="Stop"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2"/></svg></button></div><div class="volRow"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"/></svg><input id="volLoveStory" class="volSlider" type="range" min="0" max="30" value="22"><span id="musicVolLabel">22</span></div><div class="plHeader"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg> PLAYLIST DFPLAYER</div><div class="playlist"><div class="song active" data-file="DFP:1"><span>1</span><strong>Love Story</strong><small>Taylor Swift 3:54</small></div><div class="song" data-file="DFP:2"><span>2</span><strong>MBG</strong><small>Local MP3 0:15</small></div><div class="song" data-file="DFP:3"><span>3</span><strong>YELLOW</strong><small>Coldplay 4:29</small></div><div class="song" data-file="DFP:4"><span>4</span><strong>REDRED</strong><small>CORTIS 2:43</small></div></div></div>
...
<div style="grid-column:1/-1;text-align:center"><p id="drawSyncState" class="statusLine" style="margin-top:0">Live draw siap</p></div></div></div></div>
      <div id="pane-ai" class="pane"><h2 class="title">Teman Bicara</h2><p class="subtitle">Tanya atau ajak GemBot ngobrol.</p><div class="aiLayout"><div><div id="aiLimitBadge" class="pill" style="background:rgba(255,255,255,0.1);color:#fff">AI: --</div> <div id="aiKeyBadge" class="pill" style="display:none">KEY: --</div><div id="chatHistory" class="chatBox" style="height:220px;margin-top:12px;margin-bottom:12px"></div><div class="chatInput"><input id="chatInput" placeholder="Tanya GemBot..."><button id="sendChatBtn">Kirim</button></div><label style="display:block;margin-top:10px;font-weight:900;color:#fff"><input id="chatSpeak" type="checkbox" checked> Suara GemBot</label><div style="display:flex;align-items:center;justify-content:space-between;margin-top:12px;color:#fff;font-weight:900"><span>Volume AI</span><strong id="chatVoiceVolLabel">72%</strong></div><input id="chatVoiceVol" type="range" min="20" max="95" value="72" style="width:100%;margin-top:8px;touch-action:none;accent-color:#2e9bff"></div><div><p id="speechLive" class="statusLine" style="color:#fff;margin-top:0;text-align:left">Suara: <span id="inmpLevelText">0%</span> • <span id="speechStatus">SIAP</span></p><div class="chatBox" style="height:96px;margin-bottom:12px"><strong>Yang terdengar</strong><div id="heardText" style="margin-top:10px;font-size:18px;color:#172435;font-weight:900">Belum ada suara</div></div><div id="speechLog" class="chatBox" style="height:160px;margin-bottom:12px"></div><div style="display:flex;gap:10px;flex-wrap:wrap"><button id="startSpeech" class="primary">Mic Bot</button><button id="stopSpeech" class="primary">Kirim Bot</button><button id="phoneMicBtn" class="primary" style="touch-action:none;user-select:none">Tahan Mic HP</button></div><p id="phoneMicHint" class="statusLine" style="margin:10px 0 0;color:#d7e9f2;text-align:left">Tahan tombol Mic HP, bicara, lalu lepas untuk kirim.</p></div></div></div></section><div id="status" class="statusLine">System ready</div></div>${floatingChatbotHtml()}
  <script>
    if(!localStorage.getItem('owi_current_user')) location.href='/';
    const $=id=>document.getElementById(id);let currentFile='DFP:1';let musicPaused=false;let reminders=[];let manualFaceUntil=0,manualFace='●‿●';let lastServerTranscript='',volEditingUntil=0,musicBusy=false;const exprPreview={M0:['●‿●','Normal'],M1:['⌒▽⌒','Senang'],M2:['♥‿♥','Love'],M3:['＞︿＜','Marah'],M4:['O_O','Kaget'],M5:['-_-','Ngantuk'],M6:['T_T','Sedih'],M7:['✦▽✦','Excited'],M8:['¬‿¬','Smug'],M9:['o_o','Takut'],M10:['u‿u','Cozy'],M11:['@_@','Pusing'],M12:['>:3','Nakal'],M13:['///','Malu'],M14:['◉_◉','Fokus'],M15:['¬_¬','Bosan'],M16:['>_<','Nope'],M17:['♪▽♪','Party'],M18:['˘‿˘','Lega'],M19:['◑_◐','Curiga'],M20:['≧▽≦','Ketawa'],M21:['•_´•','Tekad'],M22:['O▽O','Wow'],M23:['u﹏u','Meleleh'],M24:['✧‿✧','Delight'],M25:['._.','Guilty'],M26:['◌‿◌','Daydream'],M27:['ಠ_ಠ','Grumpy'],M28:['✪▽✪','Amazed'],M29:['ಥ_ಥ','Nangis'],M30:['@﹏@','Pusing+'],M31:['>:D','Nakal+'],M32:['◑_◐','Curiga+'],M33:['⌐■‿■','Keren'],M34:['…_…','Mikir'],M35:['✧▽✧','Berbinar'],M36:['●︿●','Kecewa'],M37:['◣皿◢','Ngamuk'],M38:['¬_¬','Bosan+'],M39:['///‿///','Malu+'],M40:['?﹏?','Bingung'],M50:['♥‿♥','Touch Love'],M51:['＞▽＜','Laugh']};function setStatus(t,bad){$('status').textContent=t;$('status').style.color=bad?'#ff9aac':'#d7e9f2'}function setLiveFace(face,label,ttl=6000){manualFace=face;manualFaceUntil=Date.now()+ttl;$('faceText').textContent=face;$('faceText').classList.remove('livePulse');void $('faceText').offsetWidth;$('faceText').classList.add('livePulse');setTimeout(()=>$('faceText').classList.remove('livePulse'),180)}
    ...
    document.querySelectorAll('[data-cmd]').forEach(b=>b.onclick=async()=>{const cmd=b.dataset.cmd;const p=exprPreview[cmd];if(p)setLiveFace(p[0],p[1]);const wasDisabled=b.disabled;b.disabled=true;try{if(cmd==='P')setStatus('TAP GemBot...',false);const r=await fetch('/cmd/'+encodeURIComponent(cmd),{method:'POST'});const text=await r.text();setStatus(cmd==='P'&&r.ok?'TAP terkirim - menu/pilihan bergerak':text,!r.ok)}catch(e){setStatus(e.message,true)}finally{b.disabled=wasDisabled}});
    function selectSong(song){document.querySelectorAll('.song').forEach(x=>x.classList.remove('active'));song.classList.add('active');currentFile=song.dataset.file;$('songTitle').textContent=song.querySelector('strong').textContent;if($('songArtist'))$('songArtist').textContent=song.querySelector('small').textContent}let songDown=null;document.querySelectorAll('.song').forEach(song=>{song.addEventListener('pointerdown',e=>{songDown={x:e.clientX,y:e.clientY,el:song}});song.addEventListener('pointerup',async e=>{if(!songDown||songDown.el!==song)return;const moved=Math.hypot(e.clientX-songDown.x,e.clientY-songDown.y);songDown=null;if(moved>10)return;selectSong(song);await playCurrent()});song.addEventListener('keydown',async e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();selectSong(song);await playCurrent()}});song.tabIndex=0;song.onclick=e=>e.preventDefault()});
    function dfVol(){return Math.max(0,Math.min(30,Math.round(Number($('volLoveStory').value||22))))}
    ...
    async function dfCommand(action,track=currentTrack()){if(action==='PLAY'&&musicBusy)return false;if(action==='PLAY')musicBusy=true;try{const r=await fetch('/dfplayer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,track,volume:dfVol()})});setStatus(await r.text(),!r.ok);return r.ok}finally{if(action==='PLAY')setTimeout(()=>{musicBusy=false},500)}}
    async function playCurrent(){try{if(currentFile.startsWith('DFP:')){await dfCommand('VOL');const ok=await dfCommand('PLAY',currentTrack());if(ok){musicPaused=false;setPlayIcon('pause')}return ok}const vol=($('volLoveStory').value/100).toFixed(2);const r=await fetch('/play_audio',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({file:currentFile,volume:vol})});setStatus(await r.text(),!r.ok);if(r.ok)setPlayIcon('pause')}catch(e){setStatus(e.message,true)}}
    ...
    $('btnPrevSong').onclick=()=>{const songs=[...document.querySelectorAll('.song')];let i=songs.findIndex(x=>x.classList.contains('active'));selectSong(songs[(i+songs.length-1)%songs.length]);playCurrent()};
    $('btnNextSong').onclick=()=>{const songs=[...document.querySelectorAll('.song')];let i=songs.findIndex(x=>x.classList.contains('active'));selectSong(songs[(i+1)%songs.length]);playCurrent()};
    ...
    let volTimer=null;const volSlider=$('volLoveStory');function syncVolLabel(){volSlider.nextElementSibling.textContent=volSlider.value}['pointerdown','touchstart','focus','keydown'].forEach(ev=>volSlider.addEventListener(ev,()=>{volEditingUntil=Date.now()+2500},{passive:true}));['pointerup','touchend','blur'].forEach(ev=>volSlider.addEventListener(ev,()=>{volEditingUntil=Date.now()+1200},{passive:true}));volSlider.oninput=()=>{volEditingUntil=Date.now()+2500;syncVolLabel();if(currentFile.startsWith('DFP:')){clearTimeout(volTimer);volTimer=setTimeout(()=>dfCommand('VOL'),220)}};syncVolLabel();
    ...
    async function sendChatMessage(msg,who='Kamu'){msg=String(msg||'').trim();if(!msg)return;$('chatInput').value='';appendChat(who,msg,true);try{const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,speak:$('chatSpeak').checked,voiceVolume:($('chatVoiceVol').value/100).toFixed(2)})});const j=await r.json();if(j.error)appendChat('Error',j.error,false);else{appendChat('GemBot',j.response,false);refreshAiLimit()}}catch(e){appendChat('Error',e.message,false)}}$('sendChatBtn').onclick=()=>sendChatMessage($('chatInput').value.trim(),'Kamu');
    $('chatInput').addEventListener('keydown',e=>{if(e.key==='Enter')$('sendChatBtn').click()});const aiVolKey='gembot_ai_voice_volume';const aiVol=$('chatVoiceVol');const aiVolLabel=$('chatVoiceVolLabel');let aiVolRaf=0;function syncAiVoiceVol(){if(!aiVol||!aiVolLabel)return;aiVolLabel.textContent=aiVol.value+'%';localStorage.setItem(aiVolKey,aiVol.value)}if(aiVol){const savedAiVol=localStorage.getItem(aiVolKey);if(savedAiVol!==null)aiVol.value=Math.max(20,Math.min(95,Number(savedAiVol)||72));syncAiVoiceVol();aiVol.addEventListener('input',()=>{cancelAnimationFrame(aiVolRaf);aiVolRaf=requestAnimationFrame(syncAiVoiceVol)},{passive:true});['pointerdown','touchstart'].forEach(ev=>aiVol.addEventListener(ev,e=>e.stopPropagation(),{passive:true}))}$('logoutBtn').onclick=()=>{localStorage.removeItem('owi_current_user');location.href='/'};
    let pendingDrawCmds = [];
    ...
    function addSpeechLine(text,tag='GemBot'){if(!text)return;const line=document.createElement('div');line.style.cssText='margin:6px 0;padding:8px 10px;border-radius:10px;background:#eef6fb;color:#172435;font-weight:900';line.textContent=tag+': '+text;$('speechLog').prepend(line)}function setHeardText(text,interim=false){$('heardText').textContent=text||'Belum ada suara';$('heardText').style.opacity=interim?.65:1;$('heardText').style.color=interim?'#52677a':'#172435'}$('startSpeech').onclick=async()=>{try{$('speechStatus').textContent='MENDENGAR';setHeardText('Silakan bicara ke GemBot...',true);const r=await fetch('/voice/start',{method:'POST'});setStatus(await r.text(),!r.ok)}catch(e){setStatus(e.message,true)}};$('stopSpeech').onclick=async()=>{try{$('speechStatus').textContent='MEMAHAMI';const r=await fetch('/voice/stop',{method:'POST'});setStatus(await r.text(),!r.ok)}catch(e){setStatus(e.message,true)}};let phoneRecognition=null,phoneMicActive=false,phoneHoldWanted=false,phoneShouldSubmit=false,phoneSubmitted=false,phoneFinalText='',phoneInterimText='',phoneRestartTimer=0,phoneLastError='';function phoneSpeechApi(){return window.SpeechRecognition||window.webkitSpeechRecognition}function setPhoneHint(text,bad=false){const el=$('phoneMicHint');if(el){el.textContent=text;el.style.color=bad?'#ffb3c1':'#d7e9f2'}}async function submitPhoneTranscript(text){text=String(text||'').replace(/\s+/g,' ').trim();if(phoneSubmitted)return;phoneSubmitted=true;phoneShouldSubmit=false;if(!text){setPhoneHint('Mic HP belum menangkap teks. Tahan lebih lama dan bicara dekat HP.',true);setHeardText('Belum ada teks dari HP',true);$('speechStatus').textContent='SIAP';return}addSpeechLine(text,'HP');setPhoneHint('Teks HP dikirim ke GemBot.');$('speechStatus').textContent='MEMAHAMI';await sendChatMessage(text,'Kamu HP');$('speechStatus').textContent='SIAP'}function stopPhoneMic(){phoneHoldWanted=false;phoneShouldSubmit=true;clearTimeout(phoneRestartTimer);if(phoneRecognition&&phoneMicActive){try{phoneRecognition.stop()}catch{submitPhoneTranscript(phoneFinalText||phoneInterimText)}}else submitPhoneTranscript(phoneFinalText||phoneInterimText)}function startPhoneRecognition(resetText=true){const SR=phoneSpeechApi();if(!SR){setPhoneHint('Mic HP belum didukung di browser ini. Pakai Chrome/Edge Android.',true);setStatus('Browser tidak mendukung speech recognition.',true);return}if(resetText){phoneFinalText='';phoneInterimText='';phoneSubmitted=false;phoneShouldSubmit=false;phoneLastError=''}clearTimeout(phoneRestartTimer);phoneRecognition=new SR();phoneRecognition.lang='id-ID';phoneRecognition.interimResults=true;phoneRecognition.continuous=true;phoneRecognition.maxAlternatives=1;phoneRecognition.onstart=()=>{phoneMicActive=true;$('phoneMicBtn').textContent='Lepas untuk kirim';$('speechStatus').textContent='MIC HP';setPhoneHint('Mic HP aktif. Tetap tahan sambil bicara.');setHeardText((phoneFinalText||phoneInterimText||'Mendengarkan dari HP...').trim(),true)};phoneRecognition.onresult=e=>{let interim='';for(let i=e.resultIndex;i<e.results.length;i++){const txt=e.results[i][0].transcript||'';if(e.results[i].isFinal)phoneFinalText=(phoneFinalText+' '+txt).replace(/\s+/g,' ').trim();else interim+=txt}phoneInterimText=interim.trim()||phoneInterimText;const shown=(phoneFinalText+' '+phoneInterimText).replace(/\s+/g,' ').trim();setHeardText(shown||'Mendengarkan dari HP...',!phoneFinalText);setPhoneHint('Terdengar: '+(shown||'...'))};phoneRecognition.onerror=e=>{phoneLastError=e.error||'unknown';phoneMicActive=false;$('phoneMicBtn').textContent='Tahan Mic HP';if(phoneLastError==='no-speech'&&phoneHoldWanted){setPhoneHint('Masih mendengarkan, bicara lebih dekat HP...',true);phoneRestartTimer=setTimeout(()=>startPhoneRecognition(false),220);return}const msg=phoneLastError==='not-allowed'?'Izin mic HP ditolak browser.':('Mic HP error: '+phoneLastError);setPhoneHint(msg,true);setStatus(msg,true)};phoneRecognition.onend=()=>{phoneMicActive=false;$('phoneMicBtn').textContent='Tahan Mic HP';if(phoneHoldWanted&&!phoneShouldSubmit&&phoneLastError!=='not-allowed'){phoneRestartTimer=setTimeout(()=>startPhoneRecognition(false),180);return}if(phoneShouldSubmit)submitPhoneTranscript(phoneFinalText||phoneInterimText);else{$('speechStatus').textContent='SIAP';setPhoneHint('Tahan tombol Mic HP, bicara, lalu lepas untuk kirim.')}};try{phoneRecognition.start()}catch(e){setPhoneHint(e.message,true);setStatus(e.message,true)}}function startPhoneMicHold(e){if(e){e.preventDefault();e.stopPropagation()}if(phoneMicActive)return;phoneHoldWanted=true;phoneShouldSubmit=false;phoneSubmitted=false;phoneFinalText='';phoneInterimText='';phoneLastError='';startPhoneRecognition(true)}const phoneBtn=$('phoneMicBtn');if(phoneBtn){phoneBtn.addEventListener('pointerdown',startPhoneMicHold);['pointerup','pointercancel','pointerleave'].forEach(ev=>phoneBtn.addEventListener(ev,e=>{if(phoneHoldWanted){e.preventDefault();e.stopPropagation();stopPhoneMic()}}));phoneBtn.addEventListener('keydown',e=>{if((e.key===' '||e.key==='Enter')&&!phoneHoldWanted)startPhoneMicHold(e)});phoneBtn.addEventListener('keyup',e=>{if((e.key===' '||e.key==='Enter')&&phoneHoldWanted){e.preventDefault();stopPhoneMic()}});phoneBtn.addEventListener('contextmenu',e=>e.preventDefault())}if(!phoneSpeechApi())setPhoneHint('Mic HP butuh Chrome/Edge Android. Jika izin mic diblokir, buka link HTTPS.',true);
    ${floatingChatbotJs()}
  </script></body></html>`;
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && req.url.startsWith("/cmd/")) {
    const cmd = decodeURIComponent(req.url.slice("/cmd/".length)).slice(0, 24);
  ...
  ws.on('message', (message, isBinary) => {
    try {
      ...
      if (parsed.type === "auth" && parsed.role === "owibot") {
         ...
         voiceSessions.set(ws, { chunks: [], bytes: 0, sampleRate: 16000, processing: false, startedAt: Date.now(), lastChunkAt: Date.now() });
         latestSpeech.voiceStatus = "listening";
         logEvent("Voice start record");
      } else if (parsed.event === "stop_record") {
         const session = voiceSessions.get(ws);
         if (session) session.stopAt = Date.now();
         latestSpeech.voiceStatus = "thinking";
